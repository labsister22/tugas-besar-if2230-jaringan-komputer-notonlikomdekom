...
class BetterUDPSocket:
    ...
    def connect(self, server_addr, src_port, dest_port):
        ...
        syn_seg = Segment(src_port, dest_port, seq_num=x, flags=SYN)
        total_timeout = 30
        ...
        while True:
            ...
            # Send SYN if retransmit interval passed
            ...
            if ready[0]:
                ...
                if (synack_seg.flags & SYN) and (synack_seg.flags & ACK):
                    if synack_seg.ack_num == x + 1:
                        y = synack_seg.seq_num
                        break

            if current_time - start_time > total_timeout:
                raise TimeoutError("Handshake timeout waiting for SYN+ACK")

        # Send ACK to complete handshake
        ack_seg = Segment(src_port, dest_port, seq_num=x+1, ack_num=y+1, flags=ACK)
    ...
    def listen(self, sock, port, client_addr, syn_seg, result_queue):
        ...
        while True:
            ...
            if current_time - start_ack_time > total_timeout:
                raise TimeoutError("Handshake timeout waiting for ACK")
    ...
    def send(self, data):
        ...
        sender = GoBackNSender(self.socket, self.remote_addr)
        sender.send_data(data, self.src_port, self.dest_port)

    def receive(self):
        receiver = GoBackNReceiver(self.socket)
        data = receiver.receive_data(self.src_port, self.dest_port)
        return data
